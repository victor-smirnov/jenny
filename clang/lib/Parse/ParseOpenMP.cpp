...
static OpenMPMapModifierKind isMapModifier(Parser &P) {
  ...
  OpenMPMapModifierKind TypeModifier = static_cast<OpenMPMapModifierKind>(
      getOpenMPSimpleClauseType(OMPC_map, PP.getSpelling(Tok), P.getLangOpts().OpenMP));
...
bool Parser::ParseOpenMPVarList(OpenMPDirectiveKind DKind,
  ...
  if (Kind == OMPC_reduction || Kind == OMPC_task_reduction ||
    ...
    Data.ExtraModifier = getOpenMPSimpleClauseType(
        Kind, isIdentifier() ? PP.getSpelling(Tok) : "", getLangOpts().OpenMP);
    ...
    if (isIdentifier() && PP.LookAhead(0).is(tok::l_paren)) {
      Data.ExtraModifier = getOpenMPSimpleClauseType(Kind, PP.getSpelling(Tok), getLangOpts().OpenMP);
