...
static void HandleGreenRedAttr(TypeProcessingState &State, QualType &CurType,
                               ParsedAttr &Attr) {
  clang::Attr *A;
  if (Attr.getKind() == ParsedAttr::AT_Green)
    A = createSimpleAttr<GreenAttr>(State.getSema().Context, Attr);
  else
    A = createSimpleAttr<RedAttr>(State.getSema().Context, Attr);
...
static void processTypeAttrs(TypeProcessingState &state, QualType &type,
  ...
  for (ParsedAttr &attr : AttrsCopy) {
    ...
    case ParsedAttr::AT_Green:
    case ParsedAttr::AT_Red:
      HandleGreenRedAttr(state, type, attr);
      attr.setUsedAsTypeAttr();
      break;
