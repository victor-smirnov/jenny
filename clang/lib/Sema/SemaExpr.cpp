...
static bool isGreen(const Decl *D) {
  ...
  // Check for Green attribute on type (for lambdas and green functions).
...
ExprResult Sema::BuildResolvedCallExpr(Expr *Fn, NamedDecl *NDecl,
  ...
  if (FDecl && isGreen(FDecl) && !getLangOpts().NoGreenCode &&
    ...
    // Manually find the caller to ensure we don't skip lambdas
...
ExprResult Sema::CheckPlaceholderExpr(Expr *E) {
  ...
  case BuiltinType::BuiltinFn: {
    ...
    if (DRE) {
      ...
      } // end if (isInStdNamespace && !IsAddressable)

      if (IsAddressable) {
        // Instantiate the real function body and return a properly-typed
        // reference.
        if (FD->isImplicitlyInstantiable()) {
          InstantiateFunctionDefinition(E->getBeginLoc(), FD,
                                        /*Recursive=*/false,
