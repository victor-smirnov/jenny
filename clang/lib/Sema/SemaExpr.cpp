...
static bool isGreen(const Decl *D) {
  ...
  if (D->hasAttr<GreenAttr>())
    return true;
  if (D->hasAttr<RedAttr>())
    return false;

  // Check for Green attribute on type.
  if (const auto *FD = dyn_cast<FunctionDecl>(D)) {
    QualType T = FD->getType();
    while (const auto *AT = T->getAs<AttributedType>()) {
      if (AT->getAttrKind() == attr::Green)
        return true;
  ...
  while (DC) {
    if (const auto *ND = dyn_cast<NamespaceDecl>(DC)) {
      if (ND->hasAttr<GreenAttr>())
        return true;
      if (ND->hasAttr<RedAttr>())
        return false;
    } else if (const auto *RD = dyn_cast<CXXRecordDecl>(DC)) {
      if (RD->hasAttr<GreenAttr>())
        return true;
      if (RD->hasAttr<RedAttr>())
        return false;
    }
    DC = DC->getParent();
  }
...
ExprResult Sema::BuildResolvedCallExpr(Expr *Fn, NamedDecl *NDecl,
  ...
  // Green Fibers: Check for Red -> Green calls
  if (FDecl && isGreen(FDecl) && !getLangOpts().NoGreenCode &&
      !isUnevaluatedContext()) {
    // Manually find the caller to ensure we don't skip lambdas (which
    // getCurFunctionDecl might do)
    DeclContext *DC = CurContext;
    while (DC && !DC->isFunctionOrMethod())
      DC = DC->getParent();
    FunctionDecl *Caller = dyn_cast_or_null<FunctionDecl>(DC);
  ...
  // Green Fibers: Automatic wrapping of Green -> Red calls
  FunctionDecl *Caller = getCurFunctionDecl();
  if (FDecl && Caller && isGreen(Caller) && !isGreen(FDecl) &&
      !getLangOpts().NoGreenCode) {
    // Calling a red function from a green function - wrap in __builtin_red_call
    // Skip if this is already a builtin call (to avoid double wrapping)
    if (BuiltinID == 0 || (BuiltinID != Builtin::BI__builtin_red_call &&
                           BuiltinID != Builtin::BI__builtin_green_call)) {

      // BUGFIX: Skip wrapping for function templates and dependent calls.
      // Template functions like std::move/std::forward are non-addressable
      // in C++20, and trying to take their address causes compilation errors.
      // Also skip if the call is type-dependent (template context).
      bool IsTemplate = FDecl->getDescribedFunctionTemplate() != nullptr ||
                        FDecl->isDependentContext();
      bool IsDependent = Fn->isTypeDependent() || Fn->isValueDependent();

      if (!IsTemplate && !IsDependent) {
        // Create function pointer expression
        ExprResult FnPtr = DefaultFunctionArrayLvalueConversion(Fn);
        if (FnPtr.isInvalid())
          return ExprError();

        // Build arguments: [function_pointer, ...original_args]
        SmallVector<Expr *, 8> BuiltinArgs;
        BuiltinArgs.push_back(FnPtr.get());
        for (Expr *Arg : Args) {
          BuiltinArgs.push_back(Arg);
        }

        // Create __builtin_red_call call
        Expr *BuiltinCall = BuildBuiltinCallExpr(
            LParenLoc, Builtin::BI__builtin_red_call, BuiltinArgs);

        if (!BuiltinCall)
          return ExprError();

        // Type-check the builtin call
        CallExpr *Call = cast<CallExpr>(BuiltinCall);
        ExprResult Checked = CheckBuiltinFunctionCall(
            nullptr, Builtin::BI__builtin_red_call, Call);
...
ExprResult Sema::CheckPlaceholderExpr(Expr *E) {
  ...
  case BuiltinType::BuiltinFn: {
    ...
    if (DRE) {
      ...
      // BUGFIX: std::move and std::forward ARE addressable, despite being
      // library builtins. Exclude them from the non-addressable check.
      // This fixes Bugs #1 and #4 where our green/red wrapping code tried
      // to take their address, causing compilation errors.
      bool IsAddressable = (BuiltinID == Builtin::BImove ||
                            BuiltinID == Builtin::BImove_if_noexcept ||
                            BuiltinID == Builtin::BIforward ||
                            BuiltinID == Builtin::BIforward_like);

      if (Context.BuiltinInfo.isInStdNamespace(BuiltinID) && !IsAddressable) {
    ...
    // BUGFIX: Allow std::move/std::forward to be used as non-direct calls.
    // They are addressable library builtins that should work like normal
    // functions.
    if (DRE) {
      auto *FD = cast<FunctionDecl>(DRE->getDecl());
      unsigned BuiltinID = FD->getBuiltinID();
      bool IsAddressable = (BuiltinID == Builtin::BImove ||
                            ...
                            BuiltinID == Builtin::BIforward_like);
      if (IsAddressable) {
        // For addressable builtins like std::move/std::forward, instantiate
        // the real function body and return a properly-typed reference.
        if (FD->isImplicitlyInstantiable()) {
          InstantiateFunctionDefinition(E->getBeginLoc(), FD,
                                        /*Recursive=*/false,
                                        /*DefinitionRequired=*/true,
    ...
    // For other builtin functions, emit error
