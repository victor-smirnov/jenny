...
struct CompilationError: public ErrorInfo<CompilationError> {
  ...
  virtual void log(raw_ostream &OS) const override {
    OS << "CompilationError";
  }
...
class JennyJITImpl: public JennyJIT {
  ...
  class SymbolGenerator: public DefinitionGenerator {
  ...
  public:
    ...
    Error tryToGenerate(LookupState& LookupState, LookupKind K, JITDylib &JD,
                        ...
                        const SymbolLookupSet &LookupSet) override
    {
      ...
      if (cnt < LookupSet.size()) {
        ...
        return inprocess_->tryToGenerate(LookupState, K, JD, JDLookupFlags, LookupSet);
      }
      else {
...
public:
  ...
  std::unique_ptr<MetacallArgsAdapter> CreateMetacallArgsAdapter(
      ...
      ASTContext& Ctx0, interp::State& Info,
      SourceLocation SLoc
  ) noexcept override {
    return std::make_unique<JennyMetaCallAdapterImpl>(
      returnType, calleeReturnType, Info, Ctx0, SLoc
    );
  }
  ...
  Expected<FunctionDecl*> CreateAdapter(const CallExpr* call, llvm::ArrayRef<QualType> args) noexcept override {
    ...
    std::string buffer = "#pragma GCC diagnostic ignored \"-Wformat-nonliteral\"\n";
    buffer += "#pragma GCC diagnostic ignored \"-Wformat-security\"\n";

    buffer += "void " + fnName + "(__jy::JennyMetaCallAdapter& adapter) {\n";
    buffer += "  try {\n";
...
private:
  void AppendArgs(std::string& buffer, llvm::ArrayRef<QualType> argTypes) {
    ...
    for (QualType valType: argTypes) {
      ...
      if (valType == Ctx.JennyMetaInfoTy) {
