...
public:
  ...
  const NamedDecl* namedDecl() const noexcept override {
    return Decl;
  }
  ...
  ASTContext& context() noexcept override {
    return *PCtx;
  }

  Preprocessor& preprocessor() noexcept override {
    return *PP;
  }

  SourceManager& sourceManager() noexcept override {
    return *SM;
  }

  FileManager& fileManager() noexcept override {
    return *FM;
  }

  DiagnosticsEngine& diag() noexcept override {
    return *Diag;
  }
  ...
  virtual std::string to_ast_string() const noexcept override {
    std::string buf;
    llvm::raw_string_ostream ss(buf);
    if (Decl) {
      Decl->dump(ss);
  ...
  virtual void pretty_print() const noexcept override {
    if (Decl) {
      PrintingPolicy PP(LangOptions{});
      PP.Indentation = 1;
  ...
  virtual void ast_dump() const noexcept override {
    if (Decl) {
      Decl->dump(llvm::outs());
    }
    else {
...
std::unique_ptr<CxxDeclImpl> createCxxDeclImpl(Preprocessor& MainPP, TranslationUnitKind TUKind) {
  ...
  std::unique_ptr<ASTContext> PCtx = std::make_unique<ASTContext>(
        ...
        PP->getSelectorTable(), PP->getBuiltinInfo(), TranslationUnitKind::TU_Complete
  );
