...
private:
  APValueToMemBufferConverter(ASTContext& Ctx,
                              interp::State& state0,
                              jenny::MetaCallAllocator& Allocator,
                              SourceLocation SLoc)
    : Ctx(Ctx),
...
public:
  static Optional<ValueBuffer> convert(ASTContext& Ctx,
                                       interp::State& Info,
                                       jenny::MetaCallAllocator& Allocator,
                                       const APValue& value,
                                       QualType type,
...
private:
  ...
  void allocateBuffer(QualType type) {
    CharUnits DstSize   = Ctx.getTypeSizeInChars(type);
    CharUnits Alignment = Ctx.getTypeAlignInChars(type);
...
//      return visitReflection(Val, Ty, Offset);
      ...
      if (const Expr* init = varDecl->getInit()) {
        ...
        if (init->EvaluateAsRValue(Result, Ctx, true)) {
          if (Optional<ValueBuffer> buf = convert(Result.Val, init->getType(), init->getBeginLoc())) {
            ...
            return visitInt(ival, Ctx.UnsignedLongLongTy, Offset);
          }
        }
      }
    }
  ...
  bool visitStringLiteral(const StringLiteral* str, CharUnits Offset)
  {
    ...
    return visitInt(uval, Ctx.UnsignedLongLongTy, Offset);
  }
  ...
  bool visitRecord(const APValue &Val, QualType Ty, CharUnits Offset) {
    ...
    for (FieldDecl *FD : RD->fields()) {
      ...
      CharUnits FieldOffset =
          Ctx.toCharUnitsFromBits(FieldOffsetBits) + Offset;
  ...
  bool visitArray(const APValue &Val, QualType Ty, CharUnits Offset) {
    ...
    CharUnits ElemWidth = Ctx.getTypeSizeInChars(CAT->getElementType());
  ...
  bool visitInt(const APSInt &Val, QualType Ty, CharUnits Offset) {
    CharUnits Width = Ctx.getTypeSizeInChars(Ty);
...
//    case RK_type:
//      info = new (Ctx) ::jenny::MetaInfo(Ctx, Val.getReflectedType().getAsOpaquePtr(), MI_Type::TYPE); break;
//    case RK_expression:
//      info = new (Ctx) ::jenny::MetaInfo(Ctx, Val.getReflectedExpression(), MI_Type::EXPRESSION); break;
//    case RK_declaration:
//      info = new (Ctx) ::jenny::MetaInfo(Ctx, Val.getReflectedDeclaration(), MI_Type::DECLARATION); break;
//    case RK_base_specifier:
//      info = new (Ctx) ::jenny::MetaInfo(Ctx, Val.getReflectedBaseSpecifier(), MI_Type::BASE_SPECIFIER); break;
//    default:
//        return false;
//    }

//    Ctx.addDestruction(info);
...
{
    size_t data_size = Ctx.getTypeSizeInChars(CalleeReturnType).getQuantity();
    ValueBuffer Buffer((char*)value, CharUnits::fromQuantity(data_size));
    Result = MemBufferToAPValueConverter::convert(Ctx, Info, Buffer, CalleeReturnType, SLoc);
}
